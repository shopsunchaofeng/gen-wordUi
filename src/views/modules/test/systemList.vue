...
export default {
  ...
  methods: {
    ...
    addSystem(ind) {
      // console.log(e);
      this.$nextTick(() => {
        this.$emit("openSystempath1", this.dataForm.id);
      });
    },
    // 每页数
    ...
    changeSystem(id) {
      ...
      this.$nextTick(() => {
        this.$emit("openSystempath1", this.dataForm.id, id);
      });
    },
    init(id, num) {
      console.log("nu", num);
        ...
        if (id != "") {
          this.$http({
            ...
            if (data && data.code === 200) {
              ...
              console.log(this.dataList.length, this.systemNum);
              this.$forceUpdate();
            }
          });
        }
