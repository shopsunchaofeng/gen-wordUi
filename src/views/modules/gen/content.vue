<template>
  <div class="mod-content">
    <el-form :inline="true" :model="searchForm" @keyup.enter.native="getDataList()">
      <el-form-item>
        <el-input v-model="searchForm.name" placeholder="单位名称" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-button @click="getDataList()">查询</el-button>
        <el-button v-if="isAuth('gen:content:save')" type="primary" @click="addOrUpdateHandle()">新增单位</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="dataList" border @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
        <template slot-scope="scope">
          <el-button size="mini" @click="bianji(scope.row.id,scope.row.systemnumber,scope.row.publicparam.hylb)">编辑系统</el-button>
          <el-button v-if="isAuth('gen:danwei:remove')" type="danger" size="mini" @click="deleteHandle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination @size-change="sizeChangeHandle" @current-change="currentChangeHandle" :current-page="searchForm.page" :page-sizes="[10, 20, 50, 100]" :page-size="searchForm.limit" :total="totalPage" layout="total, sizes, prev, pager, next, jumper"></el-pagination>
    <!-- 弹窗, 新增 / 修改 -->
    ...
    <el-dialog :visible.sync="dialogVisible">
      ...
      <iframe :src="'https://view.officeapps.live.com/op/view.aspx?src='+filePath" width="100%" height="700px"></iframe>
    </el-dialog>
    <!--备案word预览-->
...
export default {
  ...
  methods: {
    ...
    bianji(id, num, hytype) {
      this.systemListVisible = true;
      this.$nextTick(() => {
        this.$refs.systemList.init(id, num, hytype);
      });
    },
    // 获取数据列表
    getDataList() {
      this.$http({
        ...
        if (data && data.code === 200) {
          for (let i = 0; i < data.page.records.length; i++) {
            data.page.records[i].publicparam = JSON.parse(data.page.records[i].publicparam)
          }
